...
open class Converter {
    ...
    protected fun convertClassParents(v: KtSuperTypeList?): List<Node.Declaration.ClassDeclarationBase.ClassParent> =
    ...
    protected fun convertConstructorClassParent(v: KtSuperTypeCallEntry) =
        Node.Declaration.ClassDeclarationBase.ConstructorClassParent(
    ...
    protected fun convertDelegationClassParent(v: KtDelegatedSuperTypeEntry) =
        Node.Declaration.ClassDeclarationBase.DelegationClassParent(
    ...
    protected fun convertTypeClassParent(v: KtSuperTypeEntry) = Node.Declaration.ClassDeclarationBase.TypeClassParent(
        type = v.typeReference?.let(::convertType)
    ...
    protected fun convertClassBody(v: KtClassBody): Node.Declaration.ClassDeclarationBase.ClassBody {
        ...
        return Node.Declaration.ClassDeclarationBase.ClassBody(
            enumEntries = ktEnumEntries.map(::convertEnumEntry),
            declarations = declarationsExcludingKtEnumEntry.map(::convertDeclaration),
        ).map(v)
    }

    protected fun convertEnumEntry(v: KtEnumEntry): Node.Declaration.ClassDeclarationBase.ClassBody.EnumEntry =
        Node.Declaration.ClassDeclarationBase.ClassBody.EnumEntry(
            modifiers = convertModifiers(v.modifierList),
    ...
    protected fun convertInitializer(v: KtAnonymousInitializer): Node.Declaration.ClassDeclarationBase.ClassBody.Initializer {
        if (v.modifierList != null) {
            throw Unsupported("Anonymous initializer with modifiers not supported")
        }
        return Node.Declaration.ClassDeclarationBase.ClassBody.Initializer(
    ...
    protected fun convertSecondaryConstructor(v: KtSecondaryConstructor) =
        Node.Declaration.ClassDeclarationBase.ClassBody.SecondaryConstructor(
            modifiers = convertModifiers(v.modifierList),
